...
public class CardService {
    ...
    public void moveToNextColumn(final long cardId, final List<BoardColumnInfoDTO> boardColumnsInfo)
        ...
        try {
            ...
            if (dto.blocked()) {
                var message = "O card de id %s está bloqueado, é necessário desbloqueá-lo para mover".formatted(cardId);
                throw new CardBlockedException(message);
            }
            var currentColumn = boardColumnsInfo.stream().filter(bc -> bc.id().equals(dto.columnId())).findFirst()
                    .orElseThrow(() -> new IllegalStateException("O card informado pertende a outro Board"));
    ...
    public void unblock(final Long id, final String reason) throws SQLException {
        try {
            ...
            var unblockDAO = new BlockDAO(connection);
